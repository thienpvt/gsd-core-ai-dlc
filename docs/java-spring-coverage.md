...
Verify requires same-phase authoritative plan evidence (`aidlc-governance-plan` source/evaluator, pass or waived status, matching phase/producer/status/timestamps with discuss, binding-rule presence on both sides, and canonical binding metadata `id`/`severity`/`summary`). Advisory rule sets and match provenance (`matchedAxis`/`matchedValue`) may differ across discuss and plan; only binding identity is correlated. Timestamps must satisfy discuss ≤ plan requested ≤ evaluated ≤ written. Missing plan evidence fails closed before any adapter runs; `plan:pre` uses `onError: halt` because plan evidence is mandatory for verify.
